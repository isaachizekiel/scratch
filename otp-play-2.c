...
int main()
{
  ...
  for(int i = 0; i < MAX_2D_ARRAY; i++)
      ...
      for (j = 0; j < MAX_MSG_SIZE; j++)
	{
      ...
      for (j = 0; j < MAX_MSG_SIZE; j++)
	{	  
	  printf("\033[0;31m%c\033[0m ", k0[j]);      
	}
      ...
      for (j = 0; j < MAX_MSG_SIZE; j++)
	{
	  printf("\033[0;34m%c\033[0m ", msgs[i][j]);
	}
  ...
  for (int j = 0; j < MAX_MSG_SIZE; j++)
    {
      ...
      if( (res[j] >= 'a' && res[j] <= 'z') || (res[j] >= 'A' && res[j] <= 'Z') || (res[j] >= '0' && res[j] <= '9'))
	{	      
	  printf("\033[0;33m%c\033[0m ", res[j]);
	}
      else
	{
	      printf("%c ", '-');
